...
// ─── Constants ────────────────────────────────────────────────────────────────
const SEO_GROUPS = ["Core", "Search", "Booking", "Account", "Blog", "Dynamic"];
...
export default function AddSeo() {
  ...
  const handleLabelChange = (val: string) => {
    set("route_label", val);
    if (!form.route_slug || form.route_slug === slugify(form.route_label)) {
      set("route_slug", slugify(val));
    }
    if (!form.meta_title) set("meta_title", val);
    if (!form.og_title) set("og_title", val);
    if (!form.twitter_title) set("twitter_title", val);
  };
  ...
  const validate = (): boolean => {
    ...
    if (!form.route_slug.trim()) errs.route_slug = "Route slug is required";
    if (!form.route_path.trim()) errs.route_path = "Route path is required";
    if (!form.route_label.trim()) errs.route_label = "Route label is required";
    if (!form.route_group) errs.route_group = "Route group is required";
    setErrors(errs);
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (payload: object) =>
      makeApiRequest(apiUrl.getAllSeo, { method: "POST", data: payload }),
    onSuccess: () => {
    ...
    onError: () => {
      toast.error("Failed to create SEO record");
    },
  });
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Add SEO Record</h1>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Route Label" icon={Globe} error={errors.route_label} required hint="Human-readable name e.g. Homepage">
                <Input
              ...
              <Field label="Route Group" icon={Tag} error={errors.route_group} required>
                <Select value={form.route_group} onValueChange={(v) => set("route_group", v)}>
                  <SelectTrigger>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field
                label="Route Slug"
                icon={Hash}
                error={errors.route_slug}
                required
                hint="URL-friendly identifier, auto-filled from label"
              >
                <Input
                  placeholder="e.g. home"
                  value={form.route_slug}
                  onChange={(e) => set("route_slug", slugify(e.target.value))}
                />
              </Field>
              ...
              <Field
                label="Route Path"
                icon={Hash}
                error={errors.route_path}
                required
                hint="Actual URL path e.g. /"
              >
                <Input
                  placeholder="e.g. /"
