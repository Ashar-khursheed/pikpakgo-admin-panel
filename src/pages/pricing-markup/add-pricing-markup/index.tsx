...
// ─── Component ────────────────────────────────────────────────────────────────
export default function AddPricingMarkup() {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (values: FormValues) =>
      makeApiRequest(apiUrl.getAllPricingMarkups, {
        method: "POST",
        data: {
          ...values,
    ...
    onSuccess: () => {
      toast.success("Pricing markup created successfully");
      queryClient.invalidateQueries({ queryKey: ["pricing-markups"] });
      navigate("/get-all-pricing-markup");
    },
    onError: () => {
      toast.error("Failed to create pricing markup");
    },
  });
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight">Add Pricing Markup</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Create a new pricing markup rule for properties
          </p>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Field label="Markup Name" icon={Tag} error={errors.name?.message} required>
              <Input
                placeholder="e.g. Standard 10% Markup"
                {...register("name")}
              />
            </Field>
