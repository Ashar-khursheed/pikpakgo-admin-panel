...
export default function EditContent() {
  ...
  const { isLoading, isError, data } = useQuery<ApiResponse>({
    queryKey: ["content-cms", id],
    queryFn: () =>
      makeApiRequest<ApiResponse>(`${apiUrl.getContentById}/${id}`),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (data) {
      ...
      setForm({
        ...
        content_html: c.content?.html ?? "",
        sections: Array.isArray(c.sections)
          ? c.sections.map((s) => ({ type: s.type ?? "", html: s.html ?? "" }))
          : [],
        ...
        no_index: c.no_index ?? false,
        schema_markup: c.schema_markup
          ? JSON.stringify(c.schema_markup, null, 2)
          : "",
        ...
        published_at: c.published_at
          ? new Date(c.published_at).toISOString().slice(0, 16)
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (payload: object) =>
      makeApiRequest(`${apiUrl.updateContent}/${id}`, {
        method: "PUT",
        data: payload,
      }),
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["content-cms", id] });
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3 text-destructive">
        ...
        <Button variant="outline" onClick={() => navigate("/content-cms")}>
          Back to listing
        </Button>
      </div>
    );
